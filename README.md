...
Navigate to **Stores → Configuration → Customers → Login as Customer → Login as Customer Settings**. Set **Enforce Shopping Assistance** to `Yes` to remove the opt-in option from the storefront account creation form.

## Usage

When enforcement is enabled, the **Allow Remote Shopping Assistance** checkbox is removed from the storefront account creation form.

In **Admin → Customers → All Customers**, the **Allow Remote Shopping Assistance** field appears editable but always saves as enabled regardless of the selected value.
